...
#define JSONSetVal(t, f)                                                       \
    f = JsonData[#f].is_null() ? (f) : JsonData[#f].get<t>()
...
class SoundFont {
  ...
  public:
    ...
    nlohmann::json GetExampleList() {
        ...
        obj["SoundFonts"][0] = {
            ConfGetVal(path),      ConfGetVal(enabled),   ConfGetVal(xgdrums),
            ConfGetVal(linattmod), ConfGetVal(lindecvol), ConfGetVal(nofx),
            ConfGetVal(minfx),     ConfGetVal(nolimits),  ConfGetVal(norampin),
            ConfGetVal(sbank),     ConfGetVal(spreset),   ConfGetVal(dbank),
            ConfGetVal(dpreset),   ConfGetVal(dbanklsb),
        };
...
class SettingsModule {
  ...
  public:
    ...
    virtual bool InitConfig(bool write = false,
                            const char *pSynthName = nullptr,
                            size_t pSynthName_sz = 0);
...
class SynthModule {
  protected:
    ...
    template <class T> T *LoadSynthConfig() {
        T *ptr = nullptr;
        ...
        if (!_synthConfig) {
            ...
            if (!_synthConfig) {
                Error("Something went wrong while loading the configuration "
                      "file!",
                      true);
                return nullptr;
            }
  ...
  public:
    ...
    virtual bool SettingsManager(uint32_t setting, bool get, void *var,
                                 size_t size) {
        return false;
    }
    ...
    virtual void PlayShortEvent(uint8_t status, uint8_t param1,
                                uint8_t param2) {
        if (!ShortEvents)
            return;
    ...
    virtual void UPlayShortEvent(uint8_t status, uint8_t param1,
                                 uint8_t param2) {
        ShortEvents->Write(status, param1, param2);
    }
    ...
    virtual SynthResult Reset(uint8_t type = 0) {
        PlayShortEvent(0xFF, type, 0);
        return Ok;
    }

    virtual SynthResult TalkToSynthDirectly(uint32_t evt, uint32_t chan,
                                            uint32_t param) {
        return Ok;
    }
};
