...
#include "bassmidi/BASSSynth.hpp"
            ...
            if (lastChkTime != curChkTime) {
                ...
                if (Stop(true)) {
                    if (Start()) {
                        ...
                        Error("Something went terribly wrong while reloading "
                              "the settings!",
                              true);
...
void OmniMIDI::SynthHost::RefreshSettings() {
    ...
    Message("Settings refreshed! (renderer %d, kdmapi %d, crender %s)",
            _SHSettings->GetRenderer(), _SHSettings->IsKDMAPIEnabled(),
            ...
            if (newSynth->StartSynthModule()) {
                ...
                if (rv) {
                    if (!_HealthThread.joinable())
                        _HealthThread =
                            std::jthread(&SynthHost::HostHealthCheck, this);
                    ...
                    if (_HealthThread.joinable()) {
                        ...
                        Error("_HealthThread failed. (ID: %x)", true,
                              _HealthThread.get_id());

                    if (!newSynth->StopSynthModule())
                        Fatal("StopSynthModule() failed!!!");
                }
            } else
                Error("StartSynthModule() failed! The driver will not output "
                      "audio.",
                      true);
        } else
            Error("LoadSynthModule() failed! The driver will not output audio.",
                  true);
    }
...
OmniMIDI::SynthModule *OmniMIDI::SynthHost::GetSynth() {
    ...
    case Synthesizers::External:
        newSynth = new OmniMIDI::PluginSynth(_SHSettings->GetCustomRenderer(),
                                             _SHSettings, ErrLog);
...
#else
        Error("This version of OmniMIDI has been compiled without the _NONFREE "
              "preprocessor directive. BASSMIDI will not be available.",
              true);
#endif
        break;
    ...
    if (newSynth == nullptr) {
        ...
        Error("The chosen synthesizer (Syn%d) is not available on this "
...
OmniMIDI::SynthResult OmniMIDI::SynthHost::PlayLongEvent(char *ev,
    ...
    if (!ev || size < 4 || size > MAX_MIDIHDR_BUF ||
        (uint8_t)ev[size - 1] != 0xF7)
            ...
            if (vendor < 0x80) {
                ...
                case 0x41: {
                    ...
                    if (command == Receive) {
                        ...
                        uint32_t lastPos = 0;
                        uint32_t addr =
                            (addrBlock << 16) | (synthPart << 8) | commandPart;
                        ...
                        while (addrBlock != SystemMessageEnd) {
                            if (addr == MIDIReset) {
                                ...
                                Message("Detected 0x%X! MIDI reset triggered.",
                                        MIDIReset);
                                char resetType = ev[readHead + 2];
                                return Synth->Reset(!resetType ? resetType
                                                               : vendor);
                            }
                                ...
                                case PatchCommonB: {
                                    ...
                                    case 0: {
                                        ...
                                        case 3: {
                                            ...
                                            params[pseWriteHead].status =
                                                MasterTune;
                                            for (uint32_t i = 0; i < varLen;
                                                 i++) {
                                                if (i == 1)
                                                    params[pseWriteHead]
                                                        .param1 =
                            ...
                            if (pseWriteHead != 0 && !noParams) {
                                ...
                                if (calcChecksum != checksum) {
                                    Message("Checksum invalid! Expected 0x%X, "
                                            "but got 0x%X.",
                                ...
                                Message(
                                    "Processed SysEx! (Block 0x%X, SynthPart "
                                    "0x%X, CommandPart 0x%X, Checksum 0x%X)",
                                    addrBlock, synthPart, commandPart,
                                    checksum);
                            ...
                            delete[] params;
                            Message("Received unsupported SysEx. (Block 0x%X, "
                                    "SynthPart 0x%X, CommandPart 0x%X)",
                                    addrBlock, synthPart, commandPart);
                            error = true;
                        }
